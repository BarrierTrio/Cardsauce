...
function get_current_pool(_type, _rarity, _legendary, _append, ...)
	...
	if G.GAME.starting_params.csau_jokers_rate or G.GAME.starting_params.csau_all_rate or frich then
		...
		for _, v in ipairs(pool) do
			...
			local item = G.P_CENTERS[v] or G.P_TAGS[v] or G.P_SEALS[v]

			if item and item.original_mod and item.original_mod.id == 'Cardsauce'
			and (G.GAME.starting_params.csau_all_rate or (item.set == 'Joker')) then
