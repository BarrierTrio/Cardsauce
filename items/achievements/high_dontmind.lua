local achInfo = {
    rarity = 1,
    config = {key_1 = 'j_csau_dontmind', key_2 = 'j_csau_pivot', level = 20},
    unlock_condition = function(self, args)
        return args.type == "hand_level" and (next(SMODS.find_card(self.config.key_1)) or next(SMODS.find_card(self.config.key_2)))
        and args.hand == "High Card" and args.level < self.config.level and args.level_after >= self.config.level
    end,
}

function achInfo.loc_vars(self)
    return { vars = {
        self.config.level,
        G.P_CENTERS[self.config.key_1].discovered and localize{type = 'name_text', set = 'Joker', key = self.config.key_1} or '????????',
        G.P_CENTERS[self.config.key_2].discovered and localize{type = 'name_text', set = 'Joker', key = self.config.key_2} or '????????'
    }}
end

return achInfo