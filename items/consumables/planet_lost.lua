local consumInfo = {
    name = 'Lost Galaxy',
    set = "Planet",
    config = { hand_type = 'jojobalFlushFibonacci' },
    origin = 'uzumaki',
    dependencies = {
        config = {
            ['Stands'] = true,
...
function consumInfo.in_pool(self, args)
    return (SMODS.is_poker_hand_visible('jojobal_FlushFibonacci') and to_big(G.GAME.hands.jojobal_Fibonacci.played) > to_big(0))
end

function consumInfo.set_card_type_badge(self, card, badges)
    badges[1] = create_badge(localize('k_galaxy_q'), get_type_colour(self, card), nil, 1.2)
end
